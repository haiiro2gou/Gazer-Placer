...
# if flag remains, go deep
    execute if score $1 Ashen.GazerPlacer.Detector matches 1 positioned ^ ^ ^-0.25 run function gazerplacer:detector/core/2
    execute if score $1 Ashen.GazerPlacer.Detector matches 1 positioned ^ ^ ^0.25 run function gazerplacer:detector/core/2
# if not, go loop
    scoreboard players remove $MaxDistance Ashen.GazerPlacer.Detector 1
    execute if score $MaxDistance Ashen.GazerPlacer.Detector matches 1.. positioned ^ ^ ^1 run function gazerplacer:detector/core/1
